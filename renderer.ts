...
async function predictWebcam() {
  ...
  if (lastVideoTime !== videoElement.currentTime) {
    ...
    // 両方のモデルで検出を実行
    const handDetections = handLandmarker?.detectForVideo(videoElement, startTimeMs);
    const poseDetections = poseLandmarker?.detectForVideo(videoElement, startTimeMs);
    ...
    if (poseDetections?.landmarks) {
      for (const landmarks of poseDetections.landmarks) {
        drawingUtils.drawConnectors(landmarks, PoseLandmarker.POSE_CONNECTIONS, { color: "#00B6FF", lineWidth: 5 });
        drawingUtils.drawLandmarks(landmarks, { color: "#FFB600", lineWidth: 2 });
      }
